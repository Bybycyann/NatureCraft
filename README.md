...
|              函数              |                   描述                   |                             参数                             |
| :----------------------------: | :--------------------------------------: | :----------------------------------------------------------: |
| `#naturecraft:model.set_state` | 对临近的 `link` 类型模型发送一次状态更新 |                              -                               |
|  `#naturecraft:model.summon`   |            生成指定的模型实体            | `namespace` : 模型所属命名空间<br />`id` : 模型ID<br />`command` : 调用生成事件时的追加指令, 若无推荐写入 help |

|           标签            |                             描述                             |
| :-----------------------: | :----------------------------------------------------------: |
|    `#naturecraft:reg`     | 每次重载数据包时会自动调用该标签中的函数更新模型和模板的注册数据. |
| `#naturecraft:event_tick` | 若自定义模型对象需要调用 `tick` 来管理事件需将事件函数通过该标签调用, 通过该标签调用事件函数确保函数可在上下文中接收正确的计时器数据. |

|      物品修饰器       |         描述          |
| :-------------------: | :-------------------: |
| `naturecraft:consume` |   将物品堆叠数量-1    |
| `naturecraft:single`  | 将物品堆叠数量设置为1 |

|           谓词            |                             描述                             |
| :-----------------------: | :----------------------------------------------------------: |
| `naturecraft:uid_matcher` | 验证实体的模型索引是否与 `#uid` 中存储的值一致. 用于目标选择器中快速筛选同一模型对象的实体基本单元. |
...
- 添加了 `uid_matcher` 谓词用在选择器中匹配相同uid的实体, 使用前需将目标uid存入 `#uid NatureCraft.uid` 中;
