...
#    tellraw @a ["[Debug] ",{storage:"naturecraft:main",nbt:"states.code",color:"green"},{entity:"@s",nbt:"Pos"}]

# 匹配
function naturecraft:place/init/states/link/comp
#    execute if score #boole NatureCraft.var matches 1 run tellraw @a ["[Debug] ",{storage:"naturecraft:main",nbt:"states.list[0].condition.code",color:"green"}]
