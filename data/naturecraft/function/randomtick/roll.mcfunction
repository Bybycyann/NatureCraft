...
# target标签
execute store result score #target NatureCraft.var run scoreboard players get @e[tag=NatureCraft.target,type=minecraft:marker,tag=NatureCraft.data,sort=nearest,limit=1] NatureCraft.uid
...
execute as @e[tag=NatureCraft.target,type=minecraft:marker,tag=NatureCraft.data,sort=nearest,limit=1] if data entity @s data.event.random at @s \
